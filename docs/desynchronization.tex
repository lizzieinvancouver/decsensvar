...
%emw7June: I think anything like 'its implications' and 'its consequences' sounds a little boring (not flashy enough). I think PNAS does not enforce style rules at the start so we could swicth to the old title, or try something else .. some ideas:
% Scattered spring: How temperatures near the equinox synchronize spring biological events
% How climate change may desynchronize spring events by shifting temperature regimes 
% Scattered spring and how climate change may disrupt synchrony of spring biological events 
...
%emw7June: I think we just show a subset of the data, right? If so, we should clarify that in the appendix and potentially here; if you're just using the ones that say '*_hybrid" in the Tree column, then we could say \textit{Juglans regia x nigra} hybrids ... I think in the appendix we should also mention the data were used again here: https://onlinelibrary.wiley.com/doi/10.1111/pce.12935
We show these that these additional factors are unnecessary by applying our framework to rare experimental data in which photoperiod and chilling are held fixed but temperatures vary. Leveraging results from \cite{Charrier:2011aa}, who tested the effect of different constant temperatures on walnut (\textit{Juglans regia} and \textit{Juglans regia x nigra} hybrids) tree budburst after the plants had experienced sufficient chilling to break endodormancy (stems were sampled from 30 trees, chilled at \(4^\circ\)C), we fit the model of the winter regimes (since temperatures are held constant, this experiment replicates this regime) using weighted least squares. The data show the exact nonlinear relationship in the mean and variance as predicted by the thermal-sum model (Figure \ref{fig:experiment}), further supporting the model can explain nonlinear trends, even under the extreme temperature range and controlled chilling and photoperiod conditions.

\begin{figure*}[t]
\includegraphics[width=1\linewidth]{desynchronization_files/figure-latex/experiment2} \caption{Mean time until budburst as a function of the average daily temperature ($\alpha$) in a controlled experiment conducted by Charrier et al. (2011) in which stems were sampled from 15 walnut trees (\textit{Juglans regia} and \textit{Juglans regia x nigra} hybrids) trees, chilled at \(4^\circ\)C, and then warmed in different temperatures environments (x-axis) with the same photoperiod conditions across temperatures until budburst. For each tree (facet), we show the observed data (points), mean (solid line), and 95\% prediction interval (outer grey region, approximately two standard deviations). The figure shows that the mean and variance are nonlinear functions of temperature even when biological factors such as chilling and photoperiod are held fixed. Both decrease with increasing temperatures as predicted by the thermal-sum model.}
\label{fig:experiment} %emw7June:  we need to clean up the captions and figures ... reader do not know what 'facet' is and the letter means nothing so we should probably relabel as a, b, c etc and say each subplot or panel. 
\end{figure*}

%emw7June: We don't 'conclude' much in biology ... 
Our results show that the long-standing and widely-used thermal-sum model \citep{quetelet1849letters, chuinearees} explains observed shifts in the timing of biological events such as declining sensitivity and synchrony. This finding has two important consequences. First, it challenges an extensive body of work that has attempted to revise the thermal-sum model by adding additional factors or mechanisms \citep{fu2015declining,kovaleskipreprint}. These factors can influence the timing of biological events, but our work suggests they do not themselves explain variation in sensitivity and synchrony. Further, they may lead to inaccurate forecasts if they do not properly model the basic thermal sum model. 
...
(\textit{Juglans regia} and \textit{Juglans regia x nigra} hybrids) conducted by Charrier et al.
