...
\begin{quote} % over by 20 and ending is still dull (I think we need to more of middle to have room for a better ending)
Many biological processes, including plant leafout and flowering, occur once cumulative temperatures reach a threshold (the thermal-sum model). In this way, temperatures are thought to coordinate the timing of biological events. But growing evidence suggests that as climates warm, both the advancement of spring has slowed (declining sensitivity) and the variance in the timing of spring events has increased (declining synchrony)---raising questions about the resilience of temperature-based coordination to anthropogenic climate change. To answer these questions, researchers have complicated the thermal-sum model, introducing additional factors and mechanisms (e.g., chilling and photoperiod). We consider whether such complexity is necessary. Using results from the theory of stopped random walks, we show that sensitivity and synchrony are exactly as predicted by the thermal-sum model. In particular, the theory predicts that as temperatures increase and springtime events shift from the equinox toward the winter solstice, the events themselves become less coordinated and more variable. We verify these predictions using experimental and real-world data, including 10,000 observations of common lilacs (United States, 1956-2025). We find that the apparently complex current shifts in temperature sensitivity and variance of phenology with climate change can be predicted by the thermal-sum model. Thus, any efforts to add complexity thus need to explain more than this simple model.% We conclude that the theory provides a powerful tool for understanding the thermal-sum model---particularly when considering additional complexity. % JDMay10: this feels a little weak ... EMW: I added something new, but I think it is still weak -- try to merge in some of the below from end of paper perhaps?
% Progress on understanding and forecasting phenological events with climate change may be slowed by continual efforts to explain variation already captured by the simple thermal sum model. This is especially concerning given the basic thermal-sum model predicts a general desynchronization of spring events as their timing moves closer to the winter solstice. Absent some compensating mechanism, the timing of biological events across the ecosystem---from crops to forests, and from plants to insects---will in many cases become more variable, with potential cascading consequences for industry and natural ecosystems.
\end{quote}
...
Declining sensitivity and synchrony are almost always interpreted as evidence that the long-standing thermal-sum model is now wrong or incomplete. Researchers have suggested myriad complexities or alternatives \citep{fu2015declining,wolkovich2021simple,gao2024thermal}. Yet no work has examined whether these puzzles are explained by the thermal-sum model, or how sensitivity and synchrony will continue to evolve under the model as climates warm.

We first demonstrate that the thermal-sum model is an example of a stopped random walk, and we propose a framework using the statistical tools developed for stopped random walks. We then apply the framework to both observational and experimental data. %JA: I don't think we should attempt to explain what a stopped random walk is in the intro. I think we can say instead that showing this is a random walk is a contribution of the paper.

We find that the thermal-sum model explains both declining sensitivity and synchrony, and thus the complexities and alternative approaches suggested in the literature are not necessary to account for these phenomena. In particular, our framework reveals the key mechanism that determines whether synchrony declines: the temperature regime under which accumulation occurs. For example, when temperatures increase day over day, as they often do near the equinox, springtime events tend to be tightly synchronized. But when warming shifts those events closer to the solstice, where temperatures are comparatively stationary, event times become more variable. % JA: Though the advancement of spring can theoretically increase, I don't think it's a main point if it's not something the literature has brought up and we don't see it in our own data. 

The implication for climate change is that as springtime events advance with anthropogenic warming, they may enter a new temperature regime in which the typical time between biological events in the same ecosystem increases from a few days to several weeks, a phenomenon we refer to as a `scattered spring.'


\section{Results \& Discussion}\label{resultsdiscuss}

The thermal-sum model states that a biological event occurs when cumulative temperature reach a threshold. We first show this model is an example of a stopped random walk, and we approximate the distribution of the event times using statistical arguments commonly applied in such cases \citep{siegmund1967some,woodroofe1982nonlinear,gut2009stopped}. This statistical result yields a framework that we then apply to both observational and experimental data. The Appendix provides additional details on the derivation, simulations, and data construction.

To derive the statistical result, consider the timing of a specific biological event, such as the bloom date of a bud. Let $X_i$ denote the effective temperature experienced by that bud on day $i$. We model $X_i$ under the two temperature regimes illustrated in Figure \ref{fig:model}.
...
\label{fig:model}
\end{figure*}
% EMW: I am sure the below is a question we will get. (JD also has it later.) One way would be to address it through the data and say how often transitions seem to shift earlier versus just shift up... I think you did this before though.... And/or, can we examples here from the lilac sites somehow (maybe sites in the appendix figures that match this in the caption)? The other related thing we need to address is explaining better than temperatures may start to accumulate during the winter temperatures regime with warming when they did not before .... maybe we consider adding a post warming panel or example here where we also show the base temperature and that plants are now accumulating during winter? I think we could add something above but am not quite sure.  
...
% JA15May: I would think the location of the equinox/solstice is largely due to the orbit/angle of the earth (solstice/equinox location) which is not affected by ACC, absent some type of spatial interaction. I think there's too much focus here on the two-regime model—which is an oversimplification since temperatures are better described by a sinusoid with no transition. I edited the manuscript to point out that a two-regime model with fixed transition generally captures daily temperatures, which seems supported by the Figures in the Appendix. 

The first regime is the stationary temperature regime in which the average daily temperature is approximately constant at some level $\alpha>0$. This regime is often observed in the winter months following the solstice, and we refer to temperatures in this regime as `winter temperatures.'  The second regime is the increasing temperature regime in which the average daily temperature rises at a constant rate $\beta>0$. This regime is often observed in the spring months closer to the equinox, and we refer to it as `spring warming.'  

In both regimes, effective temperature is modeled as average temperature plus noise $\epsilon_i$. That is, $X_i = \alpha + \beta i + \epsilon_i,$ where $\beta=0$ in the winter-temperature regime and $\beta>0$ in the spring-warming regime. Noise in this formulation can represent the influence of various biological factors, depending on the scale of analysis and on our evolving understanding of what triggers springtime events. At the bud level, it represents idiosyncratic variation specific to that bud, which could arise from microclimate differences \citep{schwartz2014separating}, bud color variation \citep{peaucelle2022accurate}, or internal biological differences that affect when the bud begins accumulating temperatures \citep{vanderschoot2014,pan2023epigenetic}. 

This two regime model in which temperatures switch from stationary to increasing well-describes both historic springtime temperature data (Figure \ref{fig:temp_plot-1}) and projected springtime temperature data (Figure \ref{fig:temp_plot-2}). We assume the $\epsilon_i$ are independent with mean $0$ and variance $\sigma^2$, although similar results are obtained under weak autocorrelation, exhibited in some historic and projected data. We also assume the effective temperature experienced by the bud is positive since biological limit prevents plants from accumulating very low temperatures, taken here to be $0^{\circ}C$ \citep{larcher1980} although this too is easily adjusted.

The bloom date $\nu(\tau)$ is the day $n$ on which cumulative temperature $Z_n = \sum_{i=1}^n X_i$ first reaches a plant-specific threshold $\tau>0$, that is, $\nu(\tau) = \min\{m:\, Z_m>\tau\}$. This formulation is standard in growing degree day models and, more generally, $Z_n$ is the ``forcing'' component of many process-based models of plant phenology \citep{schwartzbook}. Standard results for stopped random walks yield asymptotically normal approximations for the bloom date $\nu(\tau)$ when the threshold $\tau$ is sufficiently large that accumulation occurs over many days. (Our discussion assumes that warming occurs entirely in one of the two regimes. Similar results hold when warming can occur partially under both regimes since in such cases the second regime dominates. See Appendix for details.)

The approximations show that the mean and variance of bloom dates depend on the underlying temperature regime (i.e., values of $\alpha$ and $\beta$) during which the plant accumulates temperature. In the winter temperatures regime, average temperatures are constant during accumulation ($\beta = 0$), and cumulative temperature grows at an approximately linear rate. It follows that the expected bloom date advances with climate change in step with increasing winter temperatures, $\mathbb{E}[\nu(\tau)] \approx \tau / \alpha$, as is often described in the literature \citep{vitasse2022great}. In the spring warming regime, average temperatures increase during accumulation ($\beta > 0$), and cumulative temperatures grow quadratically. In this regime, the expected bloom date squared advances in step with spring warming, $\mathbb{E}[\nu(\tau)] \approx \sqrt{ 2 \tau / \beta}$. 

Within either regime, the relationship between bloom dates and temperature is non-linear as observed in recent papers \citep{fu2015declining,wolkovich2021simple}. Indeed, both regimes predict declining sensitivity in that the average bloom date advances at a diminishing rate when warming increases as measured by the winter temperatures $\alpha$ or spring warming $\beta$ (for the winter temperatures regime: $\frac{\partial}{\partial \alpha}\,\mathbb{E}[\nu(\tau)] \approx - \tau / \alpha^2$; for the spring warming regime: $\frac{\partial}{\partial \beta}\,\mathbb{E}[\nu(\tau)] \approx -\frac{1}{2}\sqrt{ 2\tau / \beta^{3}}$). 
...
Recall that the threshold $\tau$ is large. Thus when $\tau$ is in the denominator as in the spring warming regime, the variance is small---reflecting the fact that noise is quickly overwhelmed by the quadratic increase in cumulative temperatures that occurs during spring warming. This is how temperatures near the equinox synchronize plants (and buds within plants) with the same threshold---making idiosyncratic differences such as microclimate negligible. In contrast, when the threshold $\tau$ is in the numerator as in the winter temperatures regime, the variance is large---reflecting the fact that noise remains relatively influential in that idiosyncratic differences can translate into larger differences in threshold-crossing times.
...
Figures \ref{fig:forecast_mean} and \ref{fig:forecast_var} show that under the moderate warming scenario (SSP2-4.5), the mean and standard deviation are relatively unchanged. However, under the high warming scenario (SSP5-8.5), the mean bloom date advances moderately while the standard deviation increases dramatically in many locations. In particular, the standard deviation increases from a week to a month in parts of the Southern United States, a demonstration of the `scattered spring' that occurs when the temperature regime shifts. 
...
These results show how the thermal-sum model explains a wide variety of trends in the mean and variance of the timing of biological events, challenging a growing body of work claiming this variation either invalidates the model or justifies additional complexities \citep[e.g.,][]{fu2015declining,kovaleskipreprint}. For example, researchers have suggested that climate change may influence photoperiod or chilling (cool temperatures required to break endodormancy) and change the threshold of the thermal sum. Additional complexities such as these can always be introduced to explain newly observed trends, but our results suggest these factors are secondary and may not need to be be modeled at all. The main trends can be explained by the simple thermal-model, which research suggests holds across many organisms \citep{chuinearees}. %JA: The experiment actually shows that photoperiod and chilling are important factors that determine the bloom date. Our point is that these dynamics are secondary. I would shorten this to simply ask, can additional factors explain changing sensitivity/synchrony? We can hit our opinions on how to model later. 

To demonstrate that these additional factors are of secondary importance, we examine experimental data from \cite{Charrier:2011aa} in which factors such as chilling and photoperiod are held constant. We find sensitivity and synchrony still change as explained by our statistical results. Specifically, the experiments tested the effect of different constant temperatures on walnut (\textit{Juglans}\textasciitilde sp.) tree budburst after the plants had experienced sufficient chilling to break endodormancy (stems were sampled from 30 trees, chilled at \(4^\circ\)C) . Since temperatures are held constant, this experiment replicates the winter temperatures regime in which the bloom date is approximately normal with mean $\tau / \alpha$ and variance $\sigma^2\,\tau / \alpha^3$, which we fit using weighted least squares. The data fit the model well, exhibiting the nonlinear relationship in the mean and variance as predicted by the theory (Figure \ref{fig:experiment}), further supporting the thermal-sum model can explain observed trends, even under the extreme temperature range and controlled chilling and photoperiod conditions in experiments. 

We conclude that the thermal-sum model explains observed shifts in the timing of biological events. In particular, it explains observed declines in sensitivity and synchrony. This finding has two important consequences. First, it challenges an extensive body of work that has attempted to revise the thermal-sum model by adding additional factors or mechanisms such as photoperiod or chilling \citep{fu2015declining,kovaleskipreprint}. These factors can of course influence the timing of biological events, but our work suggests they do not themselves explain variation in sensitivity and synchrony since the experimental data we consider exhibits these behaviors even when factors like photoperiod and chilling are held constant. 

To understand the role played by these or any additional factors, the basic relationship between event time and temperature must be modeled correctly. Indeed, the literature often assumes a linear relationship between temperature and event time with constant variance, which we have shown is inconsistent with the thermal-sum model. The framework we propose is one way to encode the thermal-sum model, on top of which complexity may be added.

The second consequence is that the thermal-sum model predicts a `scattered spring' or general desynchronization of springtime events as their timing moves closer to the winter solstice. For example, we have shown that the synchrony of lilac bloom dates, a common indicator of spring, will decline rapidly in Southern United States by the year 2100 under scenario SSP5-8.5, although species with lower $\tau$ may decline sooner. Absent some compensating mechanism, the longstanding thermal-sum model predicts that the timing of biological events across the ecosystem---from crops to forests, and from plants to insects---will in many cases become more variable, with potential cascading consequences for industry and natural ecosystems.
...
\begin{align*}
 ...
 \end{align*} 
 Both \(\mu\) and \(\sigma\) are modeled with tensor-product splines with basis dimension k = 10. The fitted functions are shown in Figures \ref{fig:lilac_te_mean1} and \ref{fig:lilac_te_mean2}. We then calculate $\tilde \alpha$ and $\tilde \beta$ for the years 2050, 2075, and 2100 using the NEX-GDDP-CMIP6 temperature projections for two scenarios: SSP2-4.5 and SSP5-8.5 \citep{thrasher2022nasa}. The parameters \(\tilde \alpha\) and \(\tilde \beta\) are shown in Figure \ref{fig:temp_plot-2} for two sites for all three years under the two scenarios in Appendix A.3. The projected mean $ \mu(\tilde \alpha,\tilde \beta)$ and standard deviation $\sigma(\tilde \alpha, \tilde \beta)$ are shown in Figures \ref{fig:forecast_mean} and \ref{fig:forecast_var}.
...
However, the approximation does not depend on the day the accumulation begins. When \(\tau\) is sufficiently large, the first finite number of terms of the cumulative sum
are negligible by comparison. That is, for any start day \(a > 0\), \[\sum_{i=1}^{\nu(\tau)}X_i-\sum_{i=a}^{\nu(\tau)}X_i = \sum_{i=1}^{a-1}X_i = O_p(1).\]
...
and continue into regime 2. But as long as a nontrivial amount of warming happens under regime 2 (i.e., the percent of time in regime 2 
does not go to 0 as \(\tau\) goes to infinity), the quadratic accumulation in regime 2 will dominate the contribution from regime 1 when \(\tau\) is sufficiently large. 
Moreover, the linearization argument used to compute the variance shows that the asymptotic variance depends only on 
the accumulation that occurs near the event time.
