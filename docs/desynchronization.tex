...
\begin{figure*}[t]
\centering
\pandocbounded{\includegraphics[keepaspectratio]{desynchronization_files/figure-latex/two_regimes.pdf}} 
...
This two regime model in which average temperatures switch from constant to increasing well-describes both historic springtime temperature data (Figure \ref{fig:temp_plot-1} in SI.3) and projected springtime temperature data (Figure \ref{fig:temp_plot-2} in SI.3). Here we assume that warming occurs entirely in one of the two regimes, although similar results hold when warming occurs under both regimes (since in such cases the spring regime dominates, see SI.1 for details.)
...
\begin{figure*}
    ...
    \begin{subfigure}[b]{0.9\textwidth}
        \centering
                \includegraphics[width=1\textwidth, trim=0 0 .85in 0, clip]{desynchronization_files/figure-latex/lilac_te_mean-2_t}
    ...
    \end{subfigure}
     ...
     \caption{Mean and log standard deviation of lilac bloom dates (n = 10,613 from Rosemartin et al. 2015, updated to 2025 using USA National Phenology Network data) by winter temperature $(\alpha)$ and spring warming $(\beta)$ as estimated by a generalized additive model. The estimated functions are shown using bivariate plots (a, top) and cross-section plots (b, bottom). The bivariate plots show the mean decreases as $\alpha$ and $\beta$ increase (left), while the standard deviation can increase or decrease depending on the relative values of $\alpha$ and $\beta$ (right). The cross-section plots show the mean decreases both as \(\alpha\) increases holding \(\beta\) constant (top left) and as \(\beta\) increases holding \(\alpha\) constant (top right), while the standard deviation can increase or decrease depending on whether \(\alpha\) increases with \(\beta\) held constant (bottom left) or \(\beta\) increases with \(\alpha\) held constant (bottom right).}
    \label{fig:lilac_te}
\end{figure*}
...
\begin{figure*}[t] %emw20Jun: edited caption below to make it clear this is our model -- not some other statistical fit
\includegraphics[width=1\linewidth]{desynchronization_files/figure-latex/experiment2} \caption{Mean time until budburst as a function of the average daily temperature ($\alpha$) in a controlled experiment conducted by Charrier et al. (2011) in which 385 buds were observed on stems sampled from 15 walnut trees (\textit{Juglans regia} and \textit{Juglans regia x nigra} hybrids) trees, chilled at \(4^\circ\)C, and then warmed in different temperatures environments (x-axis) with the same photoperiod conditions across temperatures until budburst. For each tree (panel), we show the observed dates (points), and the fitted winter regime approximation of the thermal-sum model, showing the mean (solid line), 95\% confidence interval (inner grey region, approximately two standard errors), and 95\% prediction interval (outer grey region, approximately two standard deviations). The figure highlights that the mean and variance are nonlinear functions of temperature even when biological factors such as chilling and photoperiod are held fixed. Both decrease with increasing temperatures as predicted by the thermal-sum model in the winter regime.} %emw20Jun -- if we need to cut the caption I think we can cut down to (since we say much of this in the text):
%  in which 385 buds were observed on stems sampled from 15 walnut trees (\textit{Juglans regia} and \textit{Juglans regia x nigra} hybrids) trees, chilled at \(4^\circ\)C, and then warmed in different temperatures environments (x-axis) with the same photoperiod conditions across temperatures until budburst. For each tree (panel), we show the observed dates (points), fitted mean (solid line), 95\% confidence interval (inner grey region, approximately two standard errors), and 95\% prediction interval (outer grey region, approximately two standard deviations). 
% down to ..
% showing our fitted model (solid line shows mean, inner gray shows 95\% confidence interval, and outer grey region shows 95\% prediction interval) and observed data (points) across 15 walnut trees (each panel is a tree), chilled at \(4^\circ\)C, then warmed in different temperatures environments (x-axis) with the same photoperiod conditions across temperatures until budburst.
\label{fig:experiment}
\end{figure*}

Our results show that the long-standing and widely-used thermal-sum model \citep{quetelet1849letters, chuinearees} explains observed shifts in the timing of biological events including declining sensitivity and synchrony. This finding has two important consequences. First, it challenges an extensive body of work that has attempted to revise the thermal-sum model by adding additional factors or mechanisms \citep{fu2015declining}. These factors can influence the timing of biological events, but our work suggests they do not themselves explain current trends in sensitivity and synchrony. Further, they can produce inaccurate forecasts if the factors are modeled incorrectly.
...
\begin{figure*}
   ...
   \vspace{-2em}
     ...
     \caption{Predicted mean (top) and standard deviation (bottom, log scale) of lilac bloom dates in years 2050 and 2100 across the Southern United States under temperatures projected in the NEX-GDDP-CMIP6  high-warming scenario SSP5-8.5 \citep{thrasher2022nasa}. Lighter areas indicate earlier blooms (top) or more variable blooms (bottom) due to higher temperatures. The mean bloom date advances moderately between 2050 and 2100, while the standard deviation increases by several weeks in many locations.}
...
Finally, we calculate winter temperatures $(\tilde \alpha)$ and spring warming $(\tilde \beta)$ for the years 2025, 2050, 2075, and 2100 using the NEX-GDDP-CMIP6 temperature projections for two scenarios: SSP2-4.5 and SSP5-8.5 \citep{thrasher2022nasa} (Figure \ref{fig:temp_plot-2} in SI.3). We also calculate the projected mean bloom date $ \mu(\tilde \alpha,\tilde \beta)$ and standard deviation $\sigma(\tilde \alpha, \tilde \beta)$ (Figure \ref{fig:forecast_south} and Figures \ref{fig:forecast_mean} and \ref{fig:forecast_var} in SI.3).
...
\section{S. Supporting Information}\label{s.-appendix}
...
\subsection{SI.1 Sketch of Theoretical
Results}\label{s.1-sketch-of-theoretical-results}
...
\subsection{SI.2 Additional Tables}\label{s.2-additional-tables}
...
\subsection{SI.3 Additional Figures}\label{s.3-additional-figures}
...
\subsection{SI.4 Simulations}\label{s.4-simulations}
