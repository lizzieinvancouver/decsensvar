...
% To do
% Check guidelines: https://www.britishecologicalsociety.org/wp-content/uploads/2025/10/In-person-poster-presentation-guidelines.pdf (currently it's 81.4 cm x 118.8 cm, which is A0, yay.)

\section*{Overview}

\begin{itemize}
\item A major concern with anthropogenic climate change is that increasing warming will alter biological processes, making forecasting, mitigation, and adaptation more difficult \citep{harrison2025paleoclimate}.
\item Plant phenology—the timing of recurring life history events, such as leafout and flowering—has been routinely cited as an example, with a growing literature claiming that in response to warming, plants have slowed down, become more variable, or less predictable \citep{wang2018trends,stemkovski2023disorder} 
...
\item Using datasets from across the Northern hemisphere, we find support for this result. 
\item Our findings suggests many biological events that depend on a thermal sum model may become less predictable with continued warming, with major implications for crops, wild plants and animals. 
\end{itemize}
...
\item This thermal sum model is non-linear on an untransformed scale (Figure 1), which may explain recent slow-downs in advances of spring phenology \citep{wolkovich2021simple}.
\end{itemize}

\section*{Thermal sum model as stopped random walk}

\begin{itemize}
\item The thermal sum (growing degree day) model is well-described by the theory of stopped random walks. % It depends strongly on the temperature regime over which plants accumulate forcing. 
...
\item During periods when temperatures are relatively flat (e.g. most temperate climates in the winter or experiments with constant `forcing'), spring events have high variance as the error of the model plays a large role in determining the timing.
...
% \item Thus, when the temperature threshold (required GDD, for example) is large relative to the daily fluctuation in temperatures and temperatures are increasing, the timing of events will be relatively synchronous (i.e., negligible effect of variance due to microclimate or related factors). In contrast, the timing of events during flatter temperature regimes have a large variance, amplifying microclimate variation. % (given the same underlying temperature regime and required thermal sum)
\end{itemize}
...
Using a very large dataset of lilacs from across the United States \citep{schwartz2012caprio,rosemartin2015lilac}, we estimated the shift in variance independent of the changing mean temperature using a generalized additive model (GAM), and found increasing variance as mean February temperatures pass 12 \degree C. 
...
\begin{itemize}
\item We show that, under normal conditions, the model predicts both a slow down in advances of spring events  \citep[so-called `declining sensitivity',][]{wolkovich2021simple,walde2025rethinking} and declining variance with warming. 
...
% \item New experiments that contrast differing thermal regimes could test  ...
\end{itemize}

\section*{Figure captions}

\begin{enumerate}
\item Controlled experiments across a wide temperature range show a non-linear response to temperature; data from \citet{Charrier:2011aa}, which reported a growth chamber experiment of tree branch cuttings of walnut trees (\emph{Juglans regia}).
\item The underlying temperature regime is critical to predicting the variance of spring events with warming. Here we show simulated daily temperatures contrasting flat regimes (common in winter months) vs. linear regimes (common in spring months). Lines show mean (dashed) $+$ AR(1) noise (grey).
\item Results showing earlier blooms (top) and increasing variance (bottom) with warming. We use February for consistency across datasets (other months produce similar results), splitting the years of data into three quantiles of temperatures, and estimating the standard deviation of event dates and its errors for each quantile. Cherry blossom data come from the US National Park service (see https://competition.statistics.gmu.edu/); UK oak leafout taken from \citet{vitasse2022great}; German beech data are from PEP725 \citep{pep725}, using the subset in \citet{wolkovich2021simple}.
\item Generalized additive model (GAM) results using lilac bloom data from across the United States \citep{rosemartin2015lilac}.
\end{enumerate}

\bibliographystyle{/Users/Lizzie/Documents/EndnoteRelated/Bibtex/styles/besjournals}
\bibliography{..//..//refs/wrongdoomsday.bib}
